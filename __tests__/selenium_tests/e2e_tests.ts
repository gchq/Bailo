...
describe('End to end test', () => {
  ...
  test(
    ...
    async () => {
      ...
      const deploymentUuid = deploymentUrl.split('/').slice(-1)[0]
      logger.info(`deploying uuid is ${deploymentUuid}`)

      const imageName = `${BAILO_REGISTRY}/${deploymentUuid}/${modelInfo.name}:1`
      await runCommand(
