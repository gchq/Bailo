...
describe('services > importers > imageImporter', () => {
  ...
  test('importCompressedRegistryImage > skip non-file entry', async () => {
    ...
    expect(registryMocks.doesImageLayerExist).toBeCalledTimes(0)
    expect(typeguardMocks.hasKeysOfType).toBeCalledTimes(0)
  ...
  test('importCompressedRegistryImage > blob skip existing success', async () => {
    ...
    expect(streamPromisesMocks.finished).toBeCalledTimes(0)
    expect(typeguardMocks.hasKeysOfType).toBeCalledTimes(0)
    expect(registryMocks.putImageManifest).toBeCalledTimes(0)
    expect(result).toMatchSnapshot()
  })
  ...
  test('importCompressedRegistryImage > invalid distributionPackageName', async () => {
    ...
    expect(registryMocks.splitDistributionPackageName).toBeCalledTimes(1)
    expect(tarballMock.extractTarGzStream).toBeCalledTimes(0)
    expect(mockJson.json).toBeCalledTimes(0)
    expect(registryMocks.doesImageLayerExist).toBeCalledTimes(0)
    expect(typeguardMocks.hasKeysOfType).toBeCalledTimes(0)
    expect(registryMocks.putImageManifest).toBeCalledTimes(0)
