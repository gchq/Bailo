...
describe('services > importers > documentImporter', () => {
  ...
  test('importDocuments > skip non-file entry', async () => {
    ...
    expect(tarballMock.extractTarGzStream).toBeCalledTimes(1)
    expect(mockJson.json).toBeCalledTimes(0)
    expect(modelParserMock.parseModelCard).toBeCalledTimes(0)
    expect(modelMocks.saveImportedModelCard).toBeCalledTimes(0)
    expect(modelParserMock.parseRelease).toBeCalledTimes(0)
    expect(registryMocks.joinDistributionPackageName).toBeCalledTimes(0)
    expect(releaseMocks.saveImportedRelease).toBeCalledTimes(0)
    expect(modelParserMock.parseFile).toBeCalledTimes(0)
    expect(fileMocks.saveImportedFile).toBeCalledTimes(0)
    expect(modelMocks.setLatestImportedModelCard).toBeCalledTimes(1)
    expect(result).toMatchSnapshot()
  })
  ...
  test('importDocuments > error on invalid file entry name', async () => {
    ...
    expect(modelParserMock.parseFile).toBeCalledTimes(0)
    expect(fileMocks.saveImportedFile).toBeCalledTimes(0)
    expect(modelMocks.setLatestImportedModelCard).toBeCalledTimes(0)
