  ...
  async (req: Request, res: Response) => {
    ...
    if (codeFile) {
      minio.putObject(
        ...
        codeFileName,
        codeFile.fileStream,
        codeFile.fileReference.uncompressedSize
      )
    }
    ...
    if (binaryFile) {
      minio.putObject(
        config.minio.buckets.uploads,
        binaryFileName,
        binaryFile.fileStream,
        binaryFile.fileReference.uncompressedSize
      )
    }
    ...
    if (codeFile && binaryFile) {
      await VersionModel.findOneAndUpdate(
        { _id: version._id },
        { files: { rawBinaryPath: binaryFileName, rawCodePath: codeFileName } }
      )
      req.log.info({ code: 'adding_file_paths', binaryFileName }, 'Adding Binary path to exports of files to version')
      await version.save()
    }

    if (versionJson.logs) {
      await VersionModel.findByIdAndUpdate({ _id: version._id }, { logs: versionJson.logs })
      req.log.info({}, `Adding build logs from previous instance`)
      await version.save()
    }
...
async function streamFileToJSON(fileRef: { bucket: string; path: string; name: string }, documentName: string) {
  try {
    ...
    throw InternalServer({ fileRef }, `Internal server occurred when reading uploaded document`)
  }
}
