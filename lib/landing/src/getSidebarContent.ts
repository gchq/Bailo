...
const updateHeadingPriority = (
  ...
  if (
    childPriority &&
    childPriority > 0 &&
    (!headingPriority || childPriority < headingPriority)
  ) {
...
const extractFrontMatterFromFile = async (
...
export const extractFrontMatterFromDir = async (
  ...
  const docsMenuContent: DocsMenuContent = await docsDirectoryContents.reduce<
    Promise<DocsMenuContent>
  >(async (docsMetadata, fileOrDirName) => {
    ...
    if (stats.isFile() && fileOrDirName.endsWith(".mdx")) {
      ...
      const filePriority =
        frontMatter.priority && frontMatter.priority > 0
      ...
      headingPriority = updateHeadingPriority(
        headingPriority,
        highestChildPriority
      );
      metadata.push({
