...
@pytest.mark.mlflow
def test_import_model_from_mlflow(integration_client, mlflow_model, mlflow_uri):
...
@pytest.mark.mlflow
def test_import_nonexistent_model_from_mlflow(integration_client, mlflow_uri):
    with pytest.raises(BailoException):
        model = Model.from_mlflow(
            client=integration_client,
            mlflow_uri=mlflow_uri,
...
@pytest.mark.mlflow
def test_import_model_files_no_run(integration_client, mlflow_uri, mlflow_model_no_run):
    with pytest.raises(BailoException):
        model = Model.from_mlflow(
            client=integration_client,
            mlflow_uri=mlflow_uri,
...
@pytest.mark.mlflow
def test_import_model_no_schema(integration_client, mlflow_uri, mlflow_model):
    with pytest.raises(BailoException):
        model = Model.from_mlflow(
            client=integration_client, mlflow_uri=mlflow_uri, team_id="Uncategorised", name=mlflow_model
        )


@pytest.mark.mlflow
def test_import_experiment_from_mlflow_and_publish(mlflow_id, example_model, mlflow_uri):
    experiment_mlflow = example_model.create_experiment()
    experiment_mlflow.from_mlflow(tracking_uri=mlflow_uri, experiment_id=mlflow_id)
