...
@bundler(flavour=ModelFlavour.MLEAP)
def mleap_bundler(model, path: str, code_paths: List[str], pip_requirements: str):
    """Bundle a MLeap model with MLflow

    Args:
        model (MLeap model): The MLeap model
...
@loader(flavour=ModelFlavour.MLEAP)
def mleap_loader(model_path: str):
    raise ModelMethodNotAvailable(
        "The model loader function has not yet been implemented for H2O models"
    )


@template(flavour=ModelFlavour.MLEAP)
def mleap_template():
    """Get the MLeap model.py template

    Returns:
        str: Path to MLeap model.py template
    """
    return resource_filename("bailoclient", "resource/templates/mleap.py")
