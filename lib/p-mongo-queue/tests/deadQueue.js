...
DeadQueue('Can create queue', async ({ db }) => {
  const queue = mongodbQueue(db, 'queue', { visibility: 1, deadQueue: 'dead-queue' })
  t.ok(queue, 'Queue created ok')
})

DeadQueue('Default dead queue limit', async ({ db }) => {
  const deadQueue = mongodbQueue(db, 'dead-queue')
  const queue = mongodbQueue(db, 'queue', { visibility: 0.1, deadQueue })
