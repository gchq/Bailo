...
describe('Deployment Assessment Suite', () => {
  ...
  before(() => {
    ...
    cy.request('POST', 'http://localhost:8080/api/v2/models', {
      ...
      newDeploymentAssessmentValuesObject.metadata.overview.modelIds = [response.body.model.id]
      cy.request(
        'POST',
        `/api/v2/model/${newDeploymentAssessmentValuesObject.metadata.overview.modelIds[0]}/setup/from-schema`,
        {
        ...
        cy.request(
          'put',
          `api/v2/model/${newDeploymentAssessmentValuesObject.metadata.overview.modelIds[0]}/model-cards`,
          {
            metadata: {
              overview: {
          ...
          expect(response.body.card).to.have.property(
            'modelId',
            newDeploymentAssessmentValuesObject.metadata.overview.modelIds[0],
          )
          cy.request('patch', `/api/v2/model/${newDeploymentAssessmentValuesObject.metadata.overview.modelIds[0]}`, {
  ...
  it('create new deployment assessment', () => {
    ...
    cy.get('[data-test=entityTextInput]').type('user').wait(500)
  ...
  it('edit new deployment assessment', () => {
    ...
    cy.get('[data-test=entityTextInput]').type('user').wait(500)
    cy.get('[role=presentation] ul li:nth-child(2)').click()
