...
describe('Model with code and binary files', () => {
  before(() => {
    ...
    cy.get('[data-test=terminalLog] > span', { timeout: 1000000 }).should('contain', 'Successfully completed build')
  })
  ...
  it('Can review, deploy and test a model', () => {
    ...
    cy.log('Checking URL has been updated')
    cy.fixture('deployment.json').then({ timeout: 100000 }, (deploymentMetadata) => {
      cy.url({ timeout: 10000 })
        .as('deploymentUrl')
        .should('contain', `/deployment/${convertNameToUrlFormat(deploymentMetadata.highLevelDetails.name)}`)
        .then({ timeout: 100000 }, async (url) => {
          ...
          cy.get('[data-test=dockerPassword]')
            .invoke('text')
            .then({ timeout: 100000 }, (dockerPassword) => {
              cy.fixture('minimal_metadata.json').then({ timeout: 100000 }, async (modelMetadata) => {
