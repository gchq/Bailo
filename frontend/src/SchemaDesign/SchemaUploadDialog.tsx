...
export default function SchemaUploadDialog({
  ...
  const [schemaUse, setSchemaUse] = useState<SchemaType>(SchemaType.UPLOAD)
      ...
      await postEndpoint(`/api/v1/schema`, data)
        ...
        .then((res) => {
          if (res === schemaName) {
            ...
            setSchemaName('')
            setSchemaReference('')
            setSchemaUse(SchemaType.UPLOAD)
            setFilename('')
            handleDialogClose()
          } else {
        ...
        <Stack spacing={2} sx={{ maxWidth: '400px' }}>
          ...
          <form onSubmit={submitForm}>
            <Stack spacing={2}>
              ...
              >
                <MenuItem value={SchemaType.UPLOAD}>Upload</MenuItem>
                <MenuItem value={SchemaType.DEPLOYMENT}>Deployment</MenuItem>
              </TextField>
