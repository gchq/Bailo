...
export default function MetricItem({ metric, onChange, onDelete }: MetricItemProps) {
  ...
  const handleMetricValueOnChange = useCallback(
    (value: string) => {
      ...
      onChange({ id: metric.id, name: metric.name, value: valueAsStringOrNumber })
    },
  ...
  return (
    <Stack>
      <Stack direction='row' spacing={1}>
        ...
        />
        <TextField size='small' value={metric.value} onChange={(e) => handleMetricValueOnChange(e.target.value)} />
