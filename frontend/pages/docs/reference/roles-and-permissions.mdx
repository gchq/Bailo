...
Two different groups are available in Bailo.

- Admin: Being an admin user grants you additional system access, enabling you to perform updates such as the management
  of schemas.
- Compliance: Being a member of this group grants you access to the metrics page.

If you wish to be added to a group, please talk with your system administrator.
...
- Add discovery tags to entries they can view
...
Signed-in users can add tags to entries they can view, including public entries where those users are not collaborators.
In the entry details, consumers use **Add tags**; users with editing permission retain the existing **Edit tags**
controls.

Adding a tag does not grant permission to remove tags, change model settings, edit a model card or change collaborators.
Private-entry visibility checks still apply. Only users with the existing entry-editing permission can remove tags.

The add-only API is `POST /api/v2/model/{modelId}/tags`, with a JSON body such as `{"tag": "computer vision"}`. Tags are
trimmed, converted to lower case, and limited to 100 characters. Adding an existing tag is safe to repeat. API tokens
require the `model:read` and `model:write` scopes for the entry. A read-only token cannot add tags.
...
- [Managing Review Roles](../administration/review-roles/managing-review-roles) - Creating and configuring review roles
  (administrators)
- [Glossary](glossary) - Definitions of all Bailo terms
