...
<Box sx={{ maxWidth: '75%', margin: 'auto' }}>
  <Image
    ...
    alt='upload new schema page with upload button'
    src={bailoResetApprovals}
  />
</Box>
...
### Deployment assessment schemas

When uploading a deployment assessment schema, mandatory questions will be automatically added to those in your uploaded
schema. These are:

- Name
- Deployment risk owner
- Deployment risk owner justification
- Entry list

The above questions are used throughout the application, and therefore they are provided automatically as the first page
of the deployment assessment form. This allows schemas provided by Bailo administrators to be entirely customisable.
