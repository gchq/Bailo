...
// ....
  ...
  ui: {
    llmImport: {
      enabled: true,          // Enables the feature and shows the "Import from Text" button in the UI
    },
  },
}
```

When `ui.llmImport.enabled` is `false` (the default) or the configuration is absent, the import feature is completely
hidden from the user interface and the backend will reject any import requests.
