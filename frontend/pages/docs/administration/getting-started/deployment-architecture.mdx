...
- **Object Storage** (SeaweedFS, S3-compatible) - Stores model files and registry blobs. S3 stands for Simple Storage
  Service.
...
4. File uploads and downloads go through the **Backend** to **Object Storage** (SeaweedFS)
...
The Docker Compose environment uses SeaweedFS as its S3-compatible object store. SeaweedFS exposes its S3 API on
`http://localhost:8333`.

Additional user interfaces can be used to aid debugging and testing.

`http://localhost:9333` - Master UI, shows data topology and volume status.

`http://localhost:8888` - Filer UI, shows directory structure and buckets.
...
backend, NGINX, MongoDB, SeaweedFS, the container registry, and Mailcrab into one image using `supervisord`.
...
- **Object Storage (SeaweedFS/S3)** holds binary artefacts in both Docker Compose and standalone environments (model
  files, container image layers).
