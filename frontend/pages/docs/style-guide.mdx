...
- Use the `next/image` `Image` component for all images (never standard Markdown image syntax for in-page screenshots)
...
<Box sx={{ maxWidth: '75%', margin: 'auto' }}>
  <Image style={{ width: '100%', height: 'auto' }} alt='descriptive alt text' src={importedImageVariable} />
</Box>
```

- Import images at the top of the file using static imports
- Always include `style={{ width: '100%', height: 'auto' }}` so the image scales responsively within its container
