...
If you entered an incorrect Source Model ID during creation, you can update it directly from the model page before the
first import has been performed.

1. Navigate to your mirrored model
2. In the info banner at the top of the page, click the edit icon next to the Source Model ID
3. Enter the corrected Source Model ID and confirm

Once the source model has been exported and the mirrored model has received its first import, the Source Model ID is
locked and can no longer be changed. Hovering over the Source Model ID displays a message explaining that editing is no
longer available.
