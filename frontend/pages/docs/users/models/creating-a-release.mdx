...
3. The release form opens with the following fields to complete:
   ...
   - **Files** - select existing files from the model or upload new files via the upload dialog. Uploaded files are
     automatically added to the release and displayed in a folder-navigable browser.
...
4. Update the release notes, add or remove files, or update images. To remove a file from the release, use the context
   menu (⋮) on the file row and select **Remove from release** — this detaches the file without deleting it from the
   model.
5. **Save** your changes

## Related pages

- [Uploading Files](uploading-files) - Adding files to your model
- [Uploading Images](uploading-images) - Pushing container images
- [Understanding Reviews](../reviews/understanding-reviews) - The review process
- [Python Client](../programmatically-using-bailo/python-client) - Creating releases programmatically
