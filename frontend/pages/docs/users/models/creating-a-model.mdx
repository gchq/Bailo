...
- **Mirrored Model** - A read-only copy of a model from an external source (see
  [Model Mirroring](../users/model-mirroring/creating-a-mirrored-model))
- **Untrusted Model** - A model requiring additional scrutiny (if enabled - see
  [Untrusted Models](../users/untrusted-models))
- **Data Card** - Documentation for a dataset (see [Creating a Data Card](../users/data-cards/creating-a-data-card))
...
- **Create from template** - copy the model card structure and content from an existing model (see
  [Model Templating](../users/models/model-templating))
...
- [Python Client](../users/programmatically-using-bailo/python-client) - Creating models programmatically
