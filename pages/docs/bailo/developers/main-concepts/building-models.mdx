...
```javascript
const buildHandler = new BuildHandler([{ construct: exampleTaskA() }, { construct: exampleTaskB() }])
```
...
```javascript
const buildHandler = new BuildHandler([{ construct: createFolder(), props: { folderPath: '/tmp/a' } }])
```

This can be used for configuring existing constructs. One a `BuildHandler` is configured, it can be run by calling
`.process()`:
