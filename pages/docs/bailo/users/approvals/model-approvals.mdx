...
You can check the compliance state of any model by clicking on the 'Compliance' tab. Each item is coloured based on it's
...
can be updated by either editting the existing version or uploading a new version. Uploading a new version automatically
retriggers reviews. Reviews can be rerequested at any point using the 'Reset Approvals' model action:
