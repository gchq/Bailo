...
Now that you have made a model and prepared `code.zip` and `binary.zip` files, it is time to post it to Bailo. This guide
assumes you will use the UI to fill out the required model card and upload the files, but we also have an API as well as
clients in several languages to make the process more automatic.
